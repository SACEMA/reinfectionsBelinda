...
#load('output/3_posterior_90_null_l2_correctdata.RData')
...
convergence_diag_plot <- (ggplot(data.table(cbind(parameter = rownames(gd$psrf), gd$psrf)))
           ...
           + scale_y_discrete(labels = function(y) 
                      ifelse(y=='lambda' | y=='kappa', 
                        parse(text=paste0(y,'[2]')),  ###
                        parse(text=paste0("lambda[2]^\"'\"")))
           )
) 
...
lambda2_lambda_density.plot <- (ggplot(tmp) 
                                 ...
                                 + theme(panel.border = element_rect(colour = "black", fill = NA, size = 0.25)
                                         , panel.grid.minor = element_blank())
                                 + xlab(parse(text = expression(lambda[2]^"'"))) 
                                 + ylab(parse(text = expression(lambda[2]))) 
                                 + theme(legend.position = "none") 
                                 + scale_fill_gradientn(colours = rev(colorspace::heat_hcl(25)))
                                 + theme(axis.title.x=element_blank())
                                 + theme(axis.text.x=element_blank())
...
#ggsave(figS4, filename = 'output/convergence_plot_l2.tiff', width = 9, height = 9 , dpi=250)
if(grepl('RDS', target)){
