...
.args <- if (interactive()) sprintf(c(
  ...
  file.path('output', '4B_posterior_90_null.RData'),
  file.path('config_general.json'),
  file.path('output', 'convergence_plot_third.png'),
...
figS4_bottom <- (lambda_density.plot / lambda_logkappa_density.plot / lambda_ll.plot) |
...
if(grepl('RDS', target)){
  saveRDS(figS4, file = target, device="png")
}else{
  ggsave(figS4, filename = target, width = 9, height = 7, device="png")
}
