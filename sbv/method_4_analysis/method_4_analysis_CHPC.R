...
funcMakeResults <- function(){
  write("Running",file="m4_output.txt",append=TRUE)
  
  results <- list()
 
  set.seed(seed_batch-1)
  
  ts <- generate_data(4, data_source, seed_batch)
  ...
  finalMatrix <- foreach(i=1:spl,
                           .packages = c('deSolve','foreach','Rmpi','iterators',
    ...
    
                                        {
                                          ...
                                          error = function(e){
                                            write(paste0("Failure reported for ",i), file='error_m4.txt',append=TRUE)
                                          }
                                          )
                                        }
...
saveRDS(resultList, file="sbv/resultList_CHPC.RData")
