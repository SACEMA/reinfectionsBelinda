...
all_data <- getAllResults(method, 'sbv/method_3_analysis/output/final_output_data/')
saveRDS(all_data, file = 'sbv/method_3_analysis/output/all_data.RDS')


#exclude results
excluded_results <- excludeResultsAll(3, all_data)
saveRDS(excluded_results, file = 'sbv/method_3_analysis/output/all_data_excluded.RDS')
...
summarised_all <- all_data %>% group_by(pscale, pobs_1, pobs_2)
...
saveRDS(summarised_all, file = 'sbv/method_3_analysis/output/summarised_results_all.RDS')



#Get summarized matrix (for sensitivity)
summarised <- excluded_results %>% group_by(pscale, pobs_1, pobs_2)
...
saveRDS(summarised, file = 'sbv/method_3_analysis/output/summarised_results.RDS')
