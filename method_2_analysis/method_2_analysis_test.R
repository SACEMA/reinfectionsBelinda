...
funcMakeResults <- function(){
  ...
  write(paste0('ERI_MA', str(eri_ma$upp_reinf)), 'test_analysis.txt', append=TRUE)
  
  write(paste0('ts_adjusted', str(ts_adjusted[date>fit_through]$ma_reinf)), 'test_analysis.txt', append=TRUE)
  
  number_of_days <- nrow(eri_ma)
  days_diff <- ts_adjusted[date > fit_through]$ma_reinf - eri_ma$upp_reinf
  ...
  days_diff <- ts_adjusted[date > wave_split]$ma_reinf - eri_ma$upp_reinf
  days_diff[days_diff<0] <- 0
  days_diff[days_diff>0] <- 1
  ...
  results <- list(pscale=parameters.r$pscale[i]
                  ...
                  , days_diff_1 = length(days_diff[days_diff==1])
                  , date_first = which(conseq_diff==5)[1]
...
for (a in splseq){
  load(file="method_2_analysis/utils/m2_parameters.RData")
  ...
  saveRDS(finalMatrix, file=paste0("resultListTest_CHPC", a,".RDS"))
...
for (a in splseq){
  resultList = c(resultList,readRDS(file=paste0("resultListTest_CHPC", a,".RDS")))
}

saveRDS(resultList, file="resultListTest_CHPC.RData")
