...
funcMakeResults <- function(i){
  ...
  write(paste0('ERI_MA ', eri_ma$upp_reinf), 'test_analysis.txt', append=TRUE)
  write(paste0('ts_adjusted', ts[date>fit_through]$ma_reinf), 'test_analysis.txt', append=TRUE)
  write(paste0('ts_adjusted', ts_adjusted[date>fit_through]$ma_reinf), 'test_analysis.txt', append=TRUE)
...
for (a in 15:20) {
  ...
  saveRDS(list(next_results), file=paste0('method_',method,'_analysis/output/test_results/resultListTest',a,'.RDS'))
}
